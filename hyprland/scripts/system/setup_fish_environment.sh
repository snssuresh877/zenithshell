#!/usr/bin/env bash
# ==============================================================================
# 🐟 Fish Shell Precision Environment & PATH Synchronizer
# ==============================================================================
# - Configures ~/.config/fish/config.fish with universal Linux PATHs
# - Bridges bashrc / Mason / Cargo / Flatpak / Rustup / Android paths into Fish
# - Integrates Pywal dynamic color sequences, Zoxide, Starship, and ZenithShell aliases
# ==============================================================================
...
echo -e "${CYAN}================================================================"
echo "  🐟 Fish Shell Precision Environment & PATH Synchronizer"
echo -e "================================================================${RESET}\n"

FISH_DIR="$HOME/.config/fish"
mkdir -p "$FISH_DIR" "$FISH_DIR/conf.d" "$FISH_DIR/completions" "$FISH_DIR/functions"

CONFIG_FISH="$FISH_DIR/config.fish"

# Backup existing config if present
if [ -f "$CONFIG_FISH" ]; then
    cp "$CONFIG_FISH" "$CONFIG_FISH.bak"
    echo -e "${BLUE}ℹ Backed up existing config.fish to config.fish.bak${RESET}"
fi

cat << 'FISH_EOF' > "$CONFIG_FISH"
...
echo -e "${GREEN}✔ ~/.config/fish/config.fish successfully updated with all PATHs & aliases!${RESET}"
