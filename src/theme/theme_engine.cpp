...
void ThemeEngine::apply_theme(const Theme& t) {
    ...
    // Sync active theme tokens to GTK 3 & Thunar
    const char* home_dir = g_get_home_dir();
    if (home_dir) {
        fs::path gtk_dir = fs::path(home_dir) / ".config" / "gtk-3.0";
        std::error_code ec;
        fs::create_directories(gtk_dir, ec);
        if (!ec) {
            fs::path gtk_colors_file = gtk_dir / "zenith_colors.css";
            std::ofstream out(gtk_colors_file, std::ios::trunc);
            if (out.is_open()) {
                out << "/* Auto-generated by ZenithShell ThemeEngine */\n";
                out << "@define-color zenith_theme_bg " << t.background << ";\n";
                out << "@define-color zenith_theme_surface " << t.surface << ";\n";
                out << "@define-color zenith_theme_view_bg " << t.background << ";\n";
                out << "@define-color zenith_theme_accent " << t.accent << ";\n";
                out << "@define-color zenith_theme_fg " << t.text_primary << ";\n";
                out << "@define-color zenith_theme_fg_muted " << t.text_secondary << ";\n";
                out << "@define-color theme_selected_bg_color " << t.accent << ";\n";
                out << "@define-color theme_selected_fg_color #ffffff;\n";
            }
        }
    }
